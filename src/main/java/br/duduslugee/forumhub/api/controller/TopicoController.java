package br.duduslugee.forumhub.api.controller;


import br.duduslugee.forumhub.api.domain.topico.*;
...
public class TopicoController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity registrar(@RequestBody @Valid DadosDoRegistroDoTopico dados, UriComponentsBuilder uriBuilder) {
        ...
        return ResponseEntity.created(uri).body(new DadosDoDetalhamentoDoTopico(topico));
    }


    @GetMapping
    public ResponseEntity<Page<DadosDaListagemDoTopico>> listar(@PageableDefault(size = 10, sort = {"dataCriacao"}) Pageable paginacao) {
        var page = repository.findAllByAtivoTrue(paginacao).map(DadosDaListagemDoTopico::new);
    ...
    public ResponseEntity<?> listarTopico(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(new DadosDoDetalhamentoDoTopico(topico.get()));
    }


    @PutMapping("/{id}")
    @Transactional
    public ResponseEntity atualizar(@PathVariable Long id, @RequestBody DadosDaAtualizacaoDoTopico dados) {
        ...
        return ResponseEntity.ok(new DadosDoDetalhamentoDoTopico(topico));
    }
